...
function centreShape(shape){
  let xmin = shape.points[0].x,xmax = shape.points[0].x,ymin = shape.points[0].y,ymax = shape.points[0].y;
  for(let i = 0; i<shape.points.length ; i++){
    if(shape.points[i].x <xmin){xmin = shape.points[i].x;}
    if(shape.points[i].x >xmax){xmax = shape.points[i].x;}
    if(shape.points[i].y <ymin){ymin = shape.points[i].y;}
    if(shape.points[i].y >ymax){ymax = shape.points[i].y;}
  }
